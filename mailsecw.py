...
def load_report_config():
    ...
    config_path = os.path.join(script_dir, 'config', 'report_config.yaml')
    default_config = {
        'branding': {
            'logo_url': '',
            'company_name': 'MailSecWatcher',
            'footer_text': 'Made with ♥ by Enzo LE NAIR'
        },
        'colors': {
            'primary': '#1d3557',
            'secondary': '#457b9d',
...
def generate_html_report(domain, spf_result, dmarc_result, dkim_result,
                         mta_sts_result, tlsrpt_result, bimi_result,
                         spf_score, dmarc_score, dkim_score, mta_sts_score,
                         tlsrpt_score, bimi_score, total_score, grade):
    ...
    #Parse DMARC tags for template
    dmarc_tags = parse_dmarc_tags(dmarc_result) if dmarc_result else None
    ...
    data = {
        ...
        'protocols': {
            ...
            'bimi': {'result': bimi_result, 'score': bimi_score, 'max': 8}
        },
    ...
    output_dir = os.path.join(script_dir, config['output']['directory'])
...
def main():
    ...
    if options.output == "html":
        ...
        report_path = generate_html_report(
            domain, spf_result, dmarc_result, dkim_result,
            mta_sts_result, tlsrpt_result, bimi_result,
            spf_score, dmarc_score, dkim_score, mta_sts_score,
            tlsrpt_score, bimi_score, total_score, grade
        )
